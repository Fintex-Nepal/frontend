...
import { getAllBanks, getAllEmployeesByBranch, getAllNonClosedDepostAccount, withdrawalUrl } from '../../../utils/Url';
import axios from 'axios';
import Loader from '../../../utils/Helper/Loader';
import { WithDrawalTypeEnum, paymentTypeEnum } from '../../../utils/Helper/Enums';
...
const Withdrawal = () => {
    ...
    const submitHandler = (event) => {
        ...
        setShowLoader(true);
        axios
            .post(withdrawalUrl, withdrawalData, {
                headers: {
                    Authorization: 'Bearer ' + localStorage.getItem('adminToken'),
                },
            })
    ...
    return (
        <>
            ...
            <section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md">
                <h2 class="text-lg font-semibold text-gray-700 capitalize">Withdrawal</h2>
                ...
                <form onSubmit={submitHandler}>
                    <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="text-gray-700 " >Withdrawal Type</label>
                            <select onChange={(e) => {
                                onChangeHandler(e)
                                setIscashPayemnt(2)
                            }} name='withDrawalType' type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md">
                                <option>Select</option>
                                {WithDrawalTypeEnum?.map(widtdraw => (
                                    <option value={widtdraw.ID}>{widtdraw.Name}</option>
                                ))}
                            </select>
                        </div>
                        {isCashPayment === '2' && <div>
                        ...
                        <div>
                            <label class="text-gray-700 " >Withdrawal Check No</label>
                            <input onChange={onChangeHandler} type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md"
                                name='withDrawalChequeNumber'
                            />
                        </div>
                    ...
                    <div class="flex justify-end mt-6">
                        <button type='submit' class="px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600">Withdraw</button>
                    </div>
                </form>
            </section>
            <ToastContainer />
