...
import { getAllSubLedger, getSubLedgerReport } from '../../../utils/Url'
...
const SubLedgerReport = () => {
  ...
  const [allSubLedgerRecord, setAllSubLedgerRecord] = useState()
  ...
  const onSubmitHandler = (e) => {
    ...
    setShowLoader(true);
    axios
      .get(`${getSubLedgerReport}SubLedgerId=${formData?.SubLedgerId}&FromDate=${formData.fromDate}&ToDate=${formData.toDate}`, {
        headers: {
          Authorization: 'Bearer ' + localStorage.getItem('adminToken'),
        },
      })
      .then((res) => {
        setShowLoader(false);
        setShowRecord(true)
        setAllSubLedgerRecord(res.data)
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              ...
              {allSubLedgerRecord?.subLedgerTransactionReportDtos?.map((record, index) => (
                <>
                  <tr class="bg-white border-b">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                      {index + 1}
    ...
    return (
      <>
        ...
        <section class="max-w-4xl p-6 mx-auto bg-white rounded-md shadow-md ">
          <h2 class="text-lg font-semibold text-gray-700 capitalize ">SubLedegr Report</h2>
          <form onSubmit={onSubmitHandler}>
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
              <div>
                <label class="text-gray-700 " >SubLedger ID</label>
...
export default SubLedgerReport